...
# api/ is generated by generate_news.py, but guard it: a bare `git add api/`
# exits 128 when the directory does not exist ("pathspec did not match"), and
# with --ignore-errors it stages NOTHING while still reporting success — so a
# failed payload write would silently produce no commit at all. The trailing
# `|| true` above hides that exit code.
if [ -d api ]; then
    git add --ignore-errors api/ || true
fi
