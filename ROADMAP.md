...
- [ ] **Cap `.news_state.json` growth with a TTL-based pruning pass at load time** — `seen_links` is appended to on every edition with no eviction. After a few months it will have tens of thousands of entries (one per article ever seen), slowing the cross-edition Jaccard scan in `is_duplicate()` linearly. Add a pruning step in `load_state()` (or `save_state()`) that drops entries with `seen_at` older than `MAX_HISTORY_DAYS=60`, with a one-shot migration that stamps `seen_at=now` on legacy entries so the first prune is safe. Bound the file to the working set, not the lifetime of the repo. 2026-06-08.
